...
async def assess_webpage_credibility(url: str) -> CredibilityAssessment:
    ...
    try:
        ...
        if text_content and len(text_content) > 100:
            factors["content_quality"] = await _assess_content_quality(
                text_content[:2000]
            )
        ...
        return {
            ...
            "factors": factors,
            "domain": domain,
            "recommendations": _generate_credibility_recommendations(
                factors, credibility_level
            ),
        }
...
def _extract_author_info(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def _extract_publication_date(soup: bs4.BeautifulSoup, html_content: str) -> str | None:
    """
...
def _assess_citation_indicators(soup: bs4.BeautifulSoup) -> float:
    ...
    ref_sections = soup.find_all(
        ["section", "div"], class_=re.compile(r"reference|citation|bibliograph", re.I)
    )
    if ref_sections:
        citation_score += 0.3
...
def _generate_credibility_recommendations(
    factors: CredibilityFactors, level: str
) -> list[str]:
    """
    ...
    if level == "Very Low":
        recommendations.append(
            "Use with extreme caution - verify all claims with other sources"
        )
    elif level == "Low":
    ...
    if not factors.get("has_author"):
        recommendations.append(
            "No author information found - check for organizational authorship"
        )

    if not factors.get("has_publication_date"):
        recommendations.append(
            "No publication date found - information currency cannot be verified"
        )

    if not factors.get("https_enabled"):
        recommendations.append(
            "Site does not use HTTPS - be cautious with sensitive information"
        )
    ...
    if factors.get("content_quality", 0.5) < 0.4:
        recommendations.append(
            "Content quality concerns - use as supplementary source only"
        )
...
def _extract_title(soup: bs4.BeautifulSoup) -> str | None:
    """
    Extract page title.
...
def _extract_description(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def _extract_keywords(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def _extract_language(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def _extract_publisher(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def _extract_last_modified(soup: bs4.BeautifulSoup) -> str | None:
    """
...
def categorize_source(url: str, metadata: WebpageMetadata) -> SourceCategorization:
    ...
    if any(pattern in domain for pattern in [".edu", ".ac.", "university", "college"]):
        ...
        pattern in domain for pattern in ["reddit", "twitter", "facebook", "social"]
    ):
    ...
    if any(
        pattern in url.lower() for pattern in ["research", "study", "paper", "journal"]
    ):
        ...
        pattern in url.lower() for pattern in ["report", "white-paper", "publication"]
    ):
    ...
    if pub_date:
        try:
            ...
            if str(current_year) in pub_date or str(current_year - 1) in pub_date:
                ...
                str(year) in pub_date for year in range(current_year - 5, current_year)
            ):
...
def _calculate_categorization_confidence(
    categories: SourceCategories, domain: str, metadata: WebpageMetadata
) -> float:
    """
    ...
    if any(pattern in domain for pattern in CREDIBLE_DOMAINS):
        confidence += 0.3
...
def _generate_categorization_notes(
    categories: SourceCategories, _domain: str
) -> list[str]:
    """

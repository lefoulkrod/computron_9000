...
from agents.ollama.deep_research.types import ResearchSource, ResearchCitation
...
class SourceTracker:
    ...
    def register_access(self, url: str, tool_name: str, query: Optional[str] = None) -> None:
        """
    ...
    def get_accesses(self, url: Optional[str] = None) -> List[SourceAccess]:
        """
    ...
    def format_citation(self, url: str, style: str = "APA") -> Optional[ResearchCitation]:
        """
    ...
    def _format_apa_citation(self, source: ResearchSource) -> ResearchCitation:
        ...
        year = f"({source.publication_date.split('-')[0]})" if source.publication_date else "(n.d.)"
        title = source.title

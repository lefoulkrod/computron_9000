    ...
    try:
        ...
        if json_match:
            try:
                ...
                required_keys = ["sentiment", "emotional_tones", "consensus_level", 
                                "key_topics", "confidence_level", "brief_summary"]
...
async def analyze_reddit_comments_sentiment(
    ...
    result["top_comment_score"] = max(comment.score for comment in comments) if comments else 0
    result["average_comment_score"] = sum(comment.score for comment in comments) / len(comments) if comments else 0

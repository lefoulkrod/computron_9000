...
def _get_host_machine_config(user_locale: Optional[str] = None) -> FingerprintConfig:
    """
...
def _get_device_config(saved_state: SavedState) -> tuple[str, Dict[str, Any]]:
    ...
    if (saved_state.fingerprint and 
        saved_state.fingerprint.device_name in desktop_devices):
...
async def search_google(
    ...
    if state_path.exists():
        ...
        logger.debug(f"No browser state file found, creating new session")
    ...
    async def _perform_search(headless: bool = True) -> GoogleSearchResults:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await page.route("**/*", lambda route: asyncio.create_task(route.abort()) if route.request.resource_type in ["image", "font", "media", "stylesheet"] else asyncio.create_task(route.continue_()))
                ...
                if response:
                    is_blocked = is_blocked or any(pattern in response.url for pattern in sorry_patterns)
                
                if is_blocked and headless:
                    logger.warning("Detected verification page, retrying in headed mode...")
                    await page.close()
                    await context.close()
                    await browser.close()
                    return await _perform_search(headless=False)
                elif is_blocked:
                    logger.warning("Verification page detected, waiting for manual completion...")
                    await page.wait_for_url(lambda url: not any(pattern in url for pattern in sorry_patterns), timeout=timeout * 2)
                ...
                if any(pattern in search_url for pattern in sorry_patterns):
                    if headless:
                        logger.warning("Post-search verification detected, retrying in headed mode...")
                        await page.close()
                        await context.close()
                        await browser.close()
                        return await _perform_search(headless=False)
                ...
                if "verification" in str(e).lower() or "captcha" in str(e).lower():
                    if headless:

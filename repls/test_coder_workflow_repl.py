...
Supported agent keys (initial selection via -a or in REPL with /agent <key>):
    workflow, designer, planner, coder, verifier, reviewer
...
verifier wraps a single non-JSON line as a command JSON payload; the reviewer wraps a
line into a minimal review JSON context when plain text is provided.
"""
...
import json
import logging
from typing import Any  # noqa: F401 (kept for potential future extensions)
...
AGENT_KEYS = ["workflow", "designer", "planner", "coder", "verifier", "reviewer"]
...
async def _run_workflow(user_prompt: str) -> None:
    ...
    async for result in coder_agent_workflow(user_prompt):
        logger.info("Step result: %s", result)


async def _run_designer(user_prompt: str) -> None:
    design_prompt = f"Create an architecture design for the software assignment: {user_prompt}."
    resp = await system_designer_agent_tool(design_prompt)
    logger.info("%s", resp)
...
async def _run_planner(user_prompt: str) -> None:
    ...
    resp = await planner_agent_tool(plan_prompt)
    logger.info("%s", resp)
...
async def _run_coder(user_prompt: str) -> None:
    ...
    resp = await coder_agent_tool(step_input)
    logger.info("%s", resp)
...
async def _run_verifier(user_prompt: str) -> None:
    ...
    resp = await verifier_agent_tool(user_prompt)
    logger.info("%s", resp)


async def _run_reviewer(user_prompt: str) -> None:
    parsed = _maybe_parse_json(user_prompt)
    if parsed is None:
        payload = {
            "assignment": user_prompt,
            "result": "",
            "verifier": None,
        }
        user_prompt = json.dumps(payload)
        logger.info("Reviewer constructed minimal review payload from text prompt")
    resp = await reviewer_agent_tool(user_prompt)
    logger.info("%s", resp)

...
def test_browser_agent_basic_config() -> None:
    """Browser agent should be configured with a single open_url tool."""
    assert browser_agent.name == "BROWSER_AGENT"
    assert browser_agent.tools and len(browser_agent.tools) == 1
    # Tool should be callable (open_url)
    tool = browser_agent.tools[0]

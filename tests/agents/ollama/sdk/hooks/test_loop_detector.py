...
from agents.ollama.sdk.hooks import LoopDetector
...
@pytest.mark.unit
def test_no_action_for_different_calls():
...
@pytest.mark.unit
def test_triggers_on_repeated_calls():
    detector = LoopDetector(threshold=3)
    history = _FakeHistory()
    for i in range(3):
...
@pytest.mark.unit
def test_no_action_when_no_tools_called():
    detector = LoopDetector(threshold=3)
    history = _FakeHistory()

...
async def test_publish_to_sync_and_async_handlers(monkeypatch: Any) -> None:
    ...
    seen_sync: list[AgentEvent] = []
    seen_async: list[AgentEvent] = []

    def sync_handler(evt: AgentEvent) -> None:
        seen_sync.append(evt)

    async def async_handler(evt: AgentEvent) -> None:
...
async def test_subscription_context_manager_auto_unsubscribes() -> None:
    ...
    def handler(evt: AgentEvent) -> None:  # noqa: ARG001 - unused
        calls.append("called")
...
async def test_drain_waits_for_inflight_tasks() -> None:
    ...
    async def slow(evt: AgentEvent) -> None:  # noqa: ARG001 - value not used
        await asyncio.sleep(0.01)
        order.append("slow")

    def fast(evt: AgentEvent) -> None:  # noqa: ARG001 - value not used
        order.append("fast")
...
async def test_drain_is_idempotent_when_no_tasks() -> None:
    ...
    async def h(evt: AgentEvent) -> None:  # noqa: D401, ARG001
        await asyncio.sleep(0)
...
async def test_drain_does_not_raise_on_handler_error() -> None:
    ...
    async def bad(evt: AgentEvent) -> None:  # noqa: ARG001
        executed.append("bad")
        raise RuntimeError("boom")

    async def good(evt: AgentEvent) -> None:  # noqa: ARG001

...
- **Mid-session AUTH error translation.** Today an already-connected broker lets `imaplib.IMAP4.error` propagate out of verb handlers; the dispatcher's catch-all turns those into generic `INTERNAL` error frames, not `AUTH`. Two pieces of work to fully wire the mid-session auth path: (a) extend `fake_email` with a "reject next N post-LOGIN commands" toggle so the case is exercisable, (b) add error translation at the verb-dispatcher boundary (or inside the IMAP client) so `imaplib.IMAP4.error` responses with authentication semantics surface as `RpcError("AUTH", ...)`. Without this, stale-credential failures mid-session look like generic errors instead of triggering the auth-failed state machine in the supervisor and the reconnect UX on the app-server side. Broker_client already maps `AUTH`-coded frames to `IntegrationAuthFailed` — just needs the broker to actually emit them.
